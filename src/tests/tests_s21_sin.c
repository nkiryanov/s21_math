...
START_TEST(if_positive_zero_return_itself) {
  ...
  ck_assert_double_eq(sin(x), 0.0);  // To be sure std expected value
  ck_assert_int_eq(signbit(x), 0);

  ck_assert_double_eq(s21_sin(x), 0.0);
  ck_assert_int_eq(signbit(x), 0);
}
END_TEST
...
START_TEST(positive_regular) {
  ...
  ck_assert_double_eq_tol(s21_sin(x), sin(x), S21_EPS);
}
END_TEST
...
TCase *tcase_s21_sin(void) {
  ...
  tc = tcase_create("Tests for `s21_sin` implementation");
